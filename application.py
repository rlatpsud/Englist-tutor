...
subscription_key = '7b9df1a52bbd42feb2acdc64e3319cc6'
region = "koreacentral"
language = "en-US"
...
@app.route("/test/gettonguetwister", methods=["POST"])
...
@app.route("/test/getstory", methods=["POST"])
def getstory():
...
@app.route("/test/ackaud", methods=["POST"])
def ackaud():
...
@app.route("/test/gettts", methods=["POST"])
def gettts():
    reftext = request.form.get("reftext")
...
@app.route("/test/getttsforword", methods=["POST"])
def getttsforword():
